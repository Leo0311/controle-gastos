...
### Gastos
CRUD completo (descrição, valor, categoria, subcategoria opcional, data), listagem por categoria/período/mês, exportação e importação em lote via planilha `.xlsx` (ver abaixo). Cada gasto pode ser vinculado a um orçamento do mês/categoria (ou categoria+subcategoria) correspondente. Um seletor de Mês/Ano no topo (com o mês atual como padrão) filtra a listagem por período, com uma opção "Ver todos os meses" para removê-lo; um dropdown de Categoria (listando só categorias com pelo menos um gasto cadastrado, para não poluir com categorias nunca usadas) e um campo de busca por descrição em tempo real (client-side, case-insensitive, com pequeno debounce) completam o filtro — período, categoria e busca funcionam em conjunto.
