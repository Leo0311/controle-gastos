...
Na tela de Gastos, o botão "Escanear nota fiscal" abre um diálogo com duas formas de ler o QR Code impresso na NFC-e (nota fiscal de consumidor eletrônica), via `html5-qrcode` (carregada sob demanda): **Usar câmera** (padrão) abre a câmera do dispositivo ao vivo; **Enviar foto** deixa escolher uma imagem já tirada (ex: uma foto batida pelo celular e enviada pelo navegador do computador), útil quando não dá pra apontar a webcam do desktop direto pro papel. Hoje só notas da **SEFAZ-SC** (Santa Catarina) são suportadas — o diálogo deixa claro que é um atalho pra consultar a nota mais rápido, não uma extração 100% automática garantida, já que a página pública da SEFAZ-SC costuma exigir uma validação de segurança (captcha) que impede a leitura automática em algumas notas (isso varia nota a nota, é do lado da SEFAZ, não um erro do app).

Depois de decodificar o QR Code, o app tenta extrair os dados automaticamente em segundo plano (a URL é validada contra o domínio oficial `sat.sef.sc.gov.br`/homologação antes de qualquer requisição — proteção contra SSRF — e o HTML é lido via Jsoup): se der certo, o formulário "Novo gasto" já abre pré-preenchido (descrição, valor e data; categoria, subcategoria e orçamento ficam em branco pro usuário escolher, e nada é salvo sem revisar e confirmar). A extração só é considerada bem-sucedida quando estabelecimento e valor vêm realmente preenchidos e válidos (nunca um formulário parcialmente vazio) — se a extração falhar por qualquer motivo (o mais comum, por causa do captcha), o app **não** tenta abrir a nota automaticamente numa aba (navegadores mobile bloqueiam esse tipo de tentativa quase sempre, por não estar vinculada a um toque direto do usuário): em vez disso, mostra direto um aviso com o botão "Ver nota fiscal", que abre a página oficial da nota numa aba nova só quando o usuário realmente toca nele — um clique real, que sempre funciona, em qualquer navegador — junto com o formulário "Novo gasto" em branco. Falha na câmera em si (sem QR Code lido) só abre o formulário em branco.
