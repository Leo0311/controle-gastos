...
- **Monitoramento**: `GET /api/health` (público, sem autenticação) responde `{"status":"UP"}` com HTTP 200 quando a API e o banco estão no ar, e `{"status":"DOWN"}` com HTTP 503 se a conexão com o banco falha. É o endpoint que um serviço externo de uptime (UptimeRobot etc.) consulta para alertar quando a API cai. `GET /api/health/smtp` (mesmo padrão, também público) testa a conexão SMTP de verdade — handshake + autenticação no Gmail, sem enviar e-mail nenhum, resultado cacheado por 30s — e devolve 503 se falhar; é um endpoint separado de propósito, porque `/api/health` é consultado pelo próprio deploy da API (abaixo) e um SMTP instável não pode derrubar um deploy que não mexeu nisso. **Precisa de um monitor próprio no UptimeRobot (ou equivalente) apontando para ele** para o alerta sair de fato — sem isso o 503 existe, mas ninguém é avisado.
