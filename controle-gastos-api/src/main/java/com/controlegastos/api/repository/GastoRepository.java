...
public interface GastoRepository extends JpaRepository<Gasto, Integer> {
    ...
    @Query("SELECT g.categoria AS categoria, SUM(g.valor) AS total FROM Gasto g "
            + "WHERE g.usuarioId = :usuarioId GROUP BY g.categoria ORDER BY SUM(g.valor) DESC")
    List<CategoriaTotal> somarPorCategoria(@Param("usuarioId") Integer usuarioId);

    @Query("SELECT g.categoria AS categoria, SUM(g.valor) AS total FROM Gasto g "
            + "WHERE g.usuarioId = :usuarioId AND g.data BETWEEN :inicio AND :fim GROUP BY g.categoria")
