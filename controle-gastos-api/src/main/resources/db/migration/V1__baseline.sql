-- ============================================================================
-- V1__baseline.sql - baseline do Flyway.
--
-- Copia LITERAL de controle-gastos/src/main/resources/schema.sql no commit
-- 234c8cd (2026-09-07). Confere 100% com o schema de producao no Neon
-- (Postgres 18.6) - verificado por comparacao item a item de colunas, tipos,
-- defaults, os 13 CHECK, todos os indices (parciais e por expressao inclusos),
-- todas as FKs com ON DELETE, e as sequencias; seed 13 categorias / 91
-- subcategorias de sistema / 1 linha em _migracoes_pontuais.
--
-- Em banco POPULADO (Neon, dev existente) o Flyway registra esta versao como
-- BASELINE e NAO a executa (spring.flyway.baseline-on-migrate=true). Em banco
-- VAZIO (CI, dev novo) ela roda por inteiro. Todo comando e idempotente
-- (IF NOT EXISTS / ON CONFLICT / DROP IF EXISTS) - de proposito, para os dois
-- caminhos serem seguros. Mudancas daqui pra frente: V2__*.sql, V3__*.sql, ...
-- que NAO precisam mais ser idempotentes (rodam exatamente uma vez).
-- ============================================================================
